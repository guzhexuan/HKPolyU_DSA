...
def adam(w, dw, config=None):
    ...
    first_unbias = config['m'] / (1 - np.power(config['beta1'], t))
    second_unbias = config['v'] / (1 - np.power(config['beta2'], t))
    next_w = w - learning_rate * first_unbias / (np.sqrt(second_unbias) + config['epsilon'])
